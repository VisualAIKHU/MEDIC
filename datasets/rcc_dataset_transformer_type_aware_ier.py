...
def decode_caption(seq, idx_to_word):
    words = []
    for token in seq:
        if token == 0:
            continue
...
def find_image_file(base_dir, img_idx, exts=['.jpg', '.png', '.jpeg', '.JPG', '.gif']):
    for ext in exts:
        file_name = f"{img_idx}{ext}"
        file_path = os.path.join(base_dir, file_name)
        if os.path.exists(file_path):
            return file_path
    raise FileNotFoundError(f"Image file for {img_idx}{ext} not found in {base_dir}")

def find_feature_file(base_dir, img_idx, exts=['.jpg', '.png', '.jpeg', '.JPG', '.gif']):
    for ext in exts:
        file_name = f"{img_idx}{ext}"+'.npy'
...
class RCCDataset(Dataset):
    ...
    def __init__(self, cfg, split):
        ...
        for k, v in self.type_mapping.items():
            self.type_to_img[k] = set([x for x in v])
        ...
        img_to_idx_path = os.path.join(os.path.dirname(cfg.data.vocab_json), 'img_to_idx.json')
        with open(img_to_idx_path, 'r') as f:
            self.img_to_idx = json.load(f)
    ...
    def __getitem__(self, index):
        ...
        img_label_idx = self.img_to_idx[img_idx]

        # Fetch image data
        if self.split == 'train':
            if random.random() < 0.5:
                d_feat_path = find_feature_file(self.d_feat_dir, img_idx)
                d_img_path = find_image_file(self.d_img_dir, img_idx)
            else:
                d_feat_path = find_feature_file(self.d_feat_dir, img_idx)
                d_img_path = find_image_file(self.d_img_dir, img_idx)
        else:
        ...
        q_feat_path = find_feature_file(self.s_feat_dir, img_idx)
        q_img_path = find_image_file(self.s_img_dir, img_idx)
        ...
        ix1 = self.label_start_idx[img_label_idx]
        ix2 = self.label_end_idx[img_label_idx]
        n_cap = ix2 - ix1 + 1
        ...
        return (d_feature, q_feature,
                ...
                d_img_path, q_img_path, aux_label_pos)
...
def rcc_collate(batch):
    ...
    return (d_feat_batch, q_feat_batch, 
            seq_batch, label_with_ignore_batch, mask_batch,
            d_img_batch, q_img_batch, aux_label_pos_batch)
