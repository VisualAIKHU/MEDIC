...
class RCCDataset(Dataset):
    ...
    def __init__(self, cfg, split):
        ...
        self.d_feats = sorted(os.listdir(self.d_feat_dir), key=lambda x: int(x.split('.')[0]))
        self.s_feats = sorted(os.listdir(self.s_feat_dir), key=lambda x: int(x.split('.')[0]))
        ...
        self.d_imgs = sorted(os.listdir(self.d_img_dir), key=lambda x: int(x.split('.')[0]))
        self.s_imgs = sorted(os.listdir(self.s_img_dir), key=lambda x: int(x.split('.')[0]))
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'train':
            if random.random() < 0.5:
                ...
                d_feat_path = os.path.join(self.d_feat_dir, self.d_feats[img_idx])
                d_img_path = os.path.join(self.d_feat_dir, self.d_feats[img_idx])
        else:
            d_feat_path = os.path.join(self.d_feat_dir, self.d_feats[img_idx])
            d_img_path = os.path.join(self.d_img_dir, self.d_imgs[img_idx])
        ...
        return (d_feature, q_feature, seq, labels_with_ignore, \
                mask, d_img_path, q_img_path, aux_label_pos)
...
def rcc_collate(batch):
    ...
    return (d_feat_batch, q_feat_batch, seq_batch, label_with_ignore_batch, \
            mask_batch, d_img_batch, q_img_batch, aux_label_pos_batch)
