...
while t < cfg.train.max_iter:
    ...
    for i, batch in enumerate(train_loader):
        ...
        d_feats, sc_feats = d_feats.to(device), sc_feats.to(device) # torch.Size([128, 1024, 14, 14])
        ...
        with autocast():
            ...
            dirl_loss = dirl_loss_pos
            ccr_loss = ccr_loss_pos

            # 추가
            change_cls_loss = losses_pos['change_cls_loss']
            change_type_loss = losses_pos['change_type_cls_loss']
            expert_disentangle_loss = losses_pos['expert_disentangle_loss']
            expert_consistency_loss = losses_pos['expert_consistency_loss']
            total_loss = (
                speaker_loss +
                0.03 * dirl_loss +
                0.05 * ccr_loss +
                # 0.05 * change_cls_loss +
                # 0.1 * change_type_loss + 
        ...
        if t % cfg.train.snapshot_interval == 0:
            ...
            with torch.no_grad():
                ...
                for val_i, val_batch in enumerate(val_loader):
                    d_feats, sc_feats, labels, labels_with_ignore, \
                    masks, d_img_paths, sc_img_paths, aux_labels_pos = val_batch
