...
def decode_sequence_transformer(ix_to_word, seq):
    ...
    for i in range(N):
        ...
        for j in range(D):
            ...
            word = ix_to_word.get(ix, None)
            if word is None:
                continue

            if word in ['<START>', '<BOX_bef>', '<BOX_aft>']:
                continue

            txt_tokens.append(word)

        out.append(' '.join(txt_tokens))
    return out
...
def coco_gen_with_bbox_format(gen_list):
    """
    gen_list: list of dicts with keys: image_id, caption, bbox_before (optional), bbox_after (optional)
    """
    results = []
    for item in gen_list:
        ...
        if "bbox_before" in item:
            result["bbox_before"] = item["bbox_before"]
        if "bbox_after" in item:
            result["bbox_after"] = item["bbox_after"]
        results.append(result)
    return results


def coco_gen_with_bbox_save(gen_list, save_path):
    results = coco_gen_with_bbox_format(gen_list)
