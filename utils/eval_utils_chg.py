import sys
COCO_PATH = '/home/seanoh/coco-caption/'
...
def _evaluate_without_spice(coco, coco_res):
...
def score_generation(anno_file, result_file, include_spice=True):
    coco = COCO(anno_file)
    coco_res = coco.loadRes(result_file)

    if not include_spice:
        return copy.deepcopy(_evaluate_without_spice(coco, coco_res))
...
def score_generation_by_type(anno_file, result_file, type_file):
    ...
    print(f"\n per type image number (result_file: {result_file}):")

    for type, image_ids in type_dict.items():
        filtered = set(coco_res.getImgIds()).intersection(set(image_ids))
        print(f"  - {type}: {len(filtered)}개")

        if not filtered:
            print(f"skipping {type} because no images found in result_file")
            continue
...
if __name__ == '__main__':
    anno_path = '/mnt/disk1/clevr_change/data/change_captions_control_6_paraphrased.json'
    coco_gt_format_save(anno_path)
    anno_neg_path = '/mnt/disk1/clevr_change/data/no_change_captions_control_6_paraphrased.json'
    coco_gt_format_save(anno_neg_path, neg=True)

    save_path = '/mnt/disk1/clevr_change/data/total_change_captions_reformat_control_6_paraphrased.json'
