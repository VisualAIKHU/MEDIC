...
class Core(nn.Module):
    ...
    def forward(self, seq, dec_mask, diff_bef, diff_aft,
        ...
        enc_outputs = torch.cat([diff_bef, diff_aft], -1) # enc_outputs: (B, N, 4D)
...
class CCR(nn.Module):
    ...
    def sample(
            self,
            diff_bef_cat, diff_aft_cat, start_idx=2, unk_idx=1, sample_max=0):
        ...
        bsz = diff_bef_cat.size(0)
        max_cap_len = self.seq_length
        text_input_ids = diff_bef_cat.new_zeros((bsz, max_cap_len), dtype=torch.long)
        text_masks = diff_bef_cat.new_zeros(bsz, max_cap_len).float()  # zeros
        ...
        for dec_idx in range(max_cap_len):
            ...
            decoder_outputs, attention_weight, sim_loss = self.core(text_input_ids, text_masks, diff_bef_cat, diff_aft_cat, diagonal_mask=True)
